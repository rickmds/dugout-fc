...
- title: For home games use "vs [Cleaned Opponent]". For away games use "@ [Cleaned Opponent]". For training use "Training" or "Practice". For other events use a brief label. Do NOT append "(Home)" or "(Away)" to the title.${isTournament ? ' If the opponent is not yet determined (e.g. still pending a pool result), use the document\'s own placeholder verbatim, e.g. "vs Winner of Pool A" or "vs TBD". Never use round/stage wording ("Pool Play", "Group A", "Round of 16", etc.) as the opponent or title — that belongs in round_label only. If no opponent is identifiable at all, use "vs TBD".' : ''}
