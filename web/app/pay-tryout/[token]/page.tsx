...
function CheckoutForm({ accent, amount, currency, clubName, onSuccess }: {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (paymentIntent?.status === 'succeeded') {
      fetch('/api/tryout/confirm-payment', {
...
function PayTryoutContent() {
  ...
  async function proceedToPayment(consent: boolean) {
    ...
    const piRes = await fetch('/api/tryout/create-payment-intent', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
  ...
  useEffect(() => {
    ...
    (async () => {
      const res = await fetch(`/api/tryout/installment?token=${token}`);
  ...
  if (data?.has_future_installments && !clientSecret) return shell(
    <>
      <div style={{ fontSize: '10px', fontWeight: '700', color: '#6b7280', textTransform: 'uppercase', letterSpacing: '1.5px', marginBottom: '10px' }}>Tryout payment</div>
...
export default function PayTryoutPage() {
  return (
    <Suspense fallback={
