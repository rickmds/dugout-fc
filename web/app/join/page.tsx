...
function JoinContent() {
  ...
  async function handleSignup() {
    ...
    setSubmitting(true);
    // No try/catch previously meant a dropped connection left `submitting`
    // stuck true forever — "Creating account…" with no way out short of
    // reloading and losing the typed password. Wrapping the network calls
    // and moving the reset into `finally` fixes that without touching the
    // step-transition logic itself.
    try {
...
function AppStoreButton({ accent, accentText }: { accent: string; accentText: string }) {
  ...
  return (
    <>
      ...
      {/* pulsefc:// silently no-ops if the app isn't installed — safe as a
          secondary link, but not the primary CTA (that has to stay the App
          Store badge for everyone who doesn't have it yet). */}
      <a
        href="pulsefc://"
        style={{ display: 'block', textAlign: 'center', fontSize: 12, color: '#6B7280', marginTop: 10, textDecoration: 'none' }}
      >
        Already have the app? <span style={{ color: accent, fontWeight: 600 }}>Open it</span>
      </a>
    </>
  );
}
