...
export async function POST(req: NextRequest) {
  ...
  // Claim exclusive access to this payment before calling Stripe at all —
  // otherwise two concurrent refund requests (double-click, or two admins
  // acting at once) both read the same refundable balance, both pass the
  // check above, and both hit Stripe for the same amount. Only one request
  // can flip refund_pending false -> true; the loser is told to retry
  // rather than silently issuing a second real refund.
  const { data: claimed } = await supabase.from('fee_payments')
    ...
    .select('id');
  if (!claimed?.length) {
    return NextResponse.json({ error: 'A refund is already in progress for this payment. Please wait and try again.' }, { status: 409 });
  }
  ...
  try {
    ...
    if (payment.method === 'stripe') {
      ...
      // Same reasoning as the PaymentIntent idempotency key: a dropped
      // connection after Stripe already processed the refund shouldn't
      // issue a second one on retry.
      const idempotencyKey = `refund_${payment.id}_${refundAmount}_${surchargeAmount}`;
    ...
    await supabase.from('fee_payments').update({ refund_pending: false }).eq('id', payment.id);
  }
}
