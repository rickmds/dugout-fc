...
type Recipient = { email: string; name: string; player_name: string | null; selected: boolean };
...
export default function EmailPage() {
  ...
  const selectedRec = recipients.filter((r) => r.selected);
  ...
  const loadRecipients = useCallback(async () => {
    ...
    // Load from invites (parent emails tied to players)
    ...
    // Load from team_members who are parents (profiles with email via auth)
    ...
    for (const inv of inviteData ?? []) {
      if (!inv.email || seen.has(inv.email.toLowerCase())) continue;
    ...
    // Add any team members not already covered by invites
    for (const m of memberData ?? []) {
      const prof = (m.profiles as any);
      if (!prof) continue;
      // We can't get their email from profiles (it's in auth.users),
      // but if they're already in invites list they're covered.
    }
  ...
  async function handleWriteWithAI() {
    ...
    const { data, error } = await supabase.functions.invoke('write-email', {
      body: {
        ...
        tone,
        team_name: teamScope === 'all' ? (club?.name ?? '') : (teams.find((t) => t.id === selectedTeams[0])?.name ?? club?.name ?? ''),
        ...
        <div style={{ maxWidth: '760px' }}>
          {logsLoading ? (
            ...
            <div style={{ textAlign: 'center', padding: '80px 40px', background: '#fff', borderRadius: '16px', border: '1px solid #E2E8F0' }}>
              ...
              <div style={{ fontSize: '16px', fontWeight: '600', color: '#64748B' }}>No sent emails yet</div>
              <div style={{ fontSize: '13px', color: '#94A3B8', marginTop: '4px' }}>Emails you send will appear here</div>
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {logs.map((log) => {
                ...
                return (
                  <div key={log.id} style={{ background: '#fff', borderRadius: '14px', border: '1px solid #E2E8F0', overflow: 'hidden' }}>
              ...
              {teamScope === 'select' && (
                <div style={{ border: '1.5px solid #E2E8F0', borderRadius: '10px', overflow: 'hidden' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '8px 12px', borderBottom: '1px solid #F1F5F9', background: '#FAFAFA' }}>
                    ...
                    {selectedTeams.length > 0 && <span style={{ fontSize: '11px', fontWeight: '700', color: primary, background: `${primary}15`, borderRadius: '20px', padding: '2px 7px', whiteSpace: 'nowrap' }}>{selectedTeams.length} selected</span>}
                  </div>
          ...
          <div style={{ padding: '16px 18px', borderBottom: '1px solid #F1F5F9', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <span style={{ fontSize: '12px', color: '#94A3B8', background: '#F1F5F9', borderRadius: '20px', padding: '1px 8px' }}>
                {selectedRec.length}/{recipients.length}
              </span>
            </div>
          ...
          <div style={{ maxHeight: '480px', overflow: 'auto' }}>
            {loadingRec ? (
              ...
              <div style={{ padding: '32px 20px', textAlign: 'center' }}>
                <Mail size={28} color="#CBD5E1" style={{ marginBottom: '10px' }} />
                <div style={{ fontSize: '13px', fontWeight: '600', color: '#64748B', marginBottom: '4px' }}>No parent emails yet</div>
                <div style={{ fontSize: '12px', color: '#94A3B8' }}>Add players with parent emails via the Roster page</div>
              </div>
            ) : (
              recipients.map((r) => (
                <div
                  ...
                  style={{
                    ...
                    background: r.selected ? `${primary}06` : '#fff',
                    transition: 'background 0.1s',
                  }}
                  onMouseEnter={(e) => { if (!r.selected) (e.currentTarget as HTMLElement).style.background = '#FAFAFA'; }}
                  onMouseLeave={(e) => { if (!r.selected) (e.currentTarget as HTMLElement).style.background = '#fff'; }}
                >
                  <div style={{
                    width: '18px', height: '18px', borderRadius: '5px', flexShrink: 0,
                    border: `2px solid ${r.selected ? primary : '#CBD5E1'}`,
                    background: r.selected ? primary : 'transparent',
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                    transition: 'all 0.1s',
                  }}>
                    {r.selected && <Check size={11} color="#fff" strokeWidth={3} />}
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    {r.player_name && (
                      <div style={{ fontSize: '12px', fontWeight: '600', color: '#0F172A', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                        Parent of {r.player_name}
                      </div>
                    )}
