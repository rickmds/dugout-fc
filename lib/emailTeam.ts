...
// added via team_members with no invite row, exact pattern already proven in
// (tabs)/chat.tsx's handleEmailTeam), dedupes by lowercased email ACROSS all
// team ids in one pass, then makes exactly one send-team-email call. Calling
// this once per team in a propagate-to-linked-teams scenario would email a
// family with kids on two linked teams twice for the same event — always
// pass every relevant team id in one call instead of looping this function.
