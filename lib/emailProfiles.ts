...
// Sibling to sendTeamEmail (lib/emailTeam.ts) for flows that need to email a
// specific handful of people by profile id — a guest's linked parent, or the
// volunteers who confirmed for a cancelled callout — rather than a team's
// full parent list. Resolves recipients via /api/profile-emails (service
// role, club-scoped) then makes one send-team-email call.
export async function sendProfilesEmail(opts: {
  ...
  } catch { /* best-effort — never block the guest-management action */ }
}
